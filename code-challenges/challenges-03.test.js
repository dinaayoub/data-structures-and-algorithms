...
------------------------------------------------------------------------------------------------ */
function checkFirstNames(a, b) {
  //if their first name is greater than the second element's first name, we need to switch them, so return 1
  if (a.firstName > b.firstName) {
    return 1;
  }
  //if their first name is less than the second element's last name, leave them as is, so return -1
  else if (a.firstName < b.firstName) {
    return -1;
  }
  //otherwise, they have the same exact first name!
  //Time to check their ages
  else {
    return a.age - b.age;
  }
}
...
const sortPeopleBetter = (arr) => {
  arr.sort((a, b) => {
    // if their last name is greater than the second element's last name, we need to switch them, so return 1
    if (a.lastName > b.lastName) {
      return 1;
    }
    //if their last name is less than the second element's last name, leave them as is, so return -1
    else if (a.lastName < b.lastName) {
      return -1;
    }
    //otherwise, they have the same exact last name!
    //Time for some fun, so check their first names.
    else {
      return checkFirstNames(a, b);
    }
  });
  return arr;
};
