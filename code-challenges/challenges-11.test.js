...
const validatePin = (pin) => {
  // Solution code here...
  var stringPIN = String(pin);
  if (stringPIN.length ===4) {
    return stringPIN.match(/\d{4}/g);
  }
  else return false;
}
...
const validateEmail = (email) => email.match(/^\w+\.*\w+[@]\w+[.](com|net|org)$/g);
  // Solution code here...
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return phoneNumber.match(/(^[(]{1}\d{3}[)]{1}[-\s]?\d{3}[-\s]?\d{4}$)|(^\d{3}[- ]?\d{3}[- ]?\d{4}$)/g); ///(^[(]?\d{3}[)]?)|(^\d{3})[\s-]{0,1}\d{3}[ -]{0,1}\d{4}$/g);
};
