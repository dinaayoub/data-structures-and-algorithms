...
const templateWithJQuery = () => {
  starWarsPeople.forEach(item => {
    ...
    var $template = $('<section>'+html+'</section>');
...
const howMuchPencil = (str) => {
  ...
  for (var i=0; i<=str.length; i++) {
    let currentString = str.slice(i,str.length);
...
const wordsToCharList = (stringToChar) => {
  var arrayOfCharacters = [];
  // Solution code here...
  for (let i=0; i<stringToChar.length; i++) {
    arrayOfCharacters.push(stringToChar.slice(i,i+1));
  }
...
const listFoods = (recipe) => {
  ...
  for (var i=0; i<ingredients.length; i++) {
    let firstSlice = ingredients[i].slice(ingredients[i].indexOf(' ')+1);
