...
const mapCurrentEvents = () => {
  // Solution code here...
  return currentEvents.news.map( item => new Event(item));
}
...
const countNumberOfElements = (arr) => arr.reduce((accumulator, current) => accumulator + 1);
...
const reversedString = (str) => Array.from(str).reduce( (accumulator,current) => {
